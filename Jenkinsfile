      ...
      parallel {
          ...
          steps {
            container( 'jetty-build' ) {
              timeout( time: 120, unit: 'MINUTES' ) {
                mavenBuild( "jdk11", "-T3 clean install", "maven3", true, [[parserName: 'Maven'], [parserName: 'Java']] ) // -Pautobahn
        ...
        stage( "Build / Test - JDK14" ) {
          ...
          steps {
            container( 'jetty-build' ) {
              timeout( time: 120, unit: 'MINUTES' ) {
                mavenBuild( "jdk14", "-T3 clean install", "maven3", true, warnings consoleParsers: [[parserName: 'Maven'], [parserName: 'Java']] )
              }
            }
          }
        }
            ...
            container( 'jetty-build' ) {
              timeout( time: 30, unit: 'MINUTES' ) {
                mavenBuild( "jdk11",
                            ...
                            "maven3", false, [[parserName: 'Maven'], [parserName: 'JavaDoc'], [parserName: 'Java']] )
              }
            }
          }
        }
            ...
            container( 'jetty-build' ) {
              timeout( time: 30, unit: 'MINUTES' ) {
                mavenBuild( "jdk11", "-T3 -Pcompact3 clean install -DskipTests", "maven3",
                            false, [[parserName: 'Maven'], [parserName: 'Java']])
              }
            }
          }
        }
 ...
 */
def mavenBuild(jdk, cmdline, mvnName, junitParsing, consoleParsers) {
  script {
    try {
      ...
      withMaven(
        ...
        options: [junitPublisher(disabled: true), mavenLinkerPublisher(disabled: false), pipelineGraphPublisher(disabled: false)],
        mavenOpts: mavenOpts,
